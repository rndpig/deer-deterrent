#!/usr/bin/env python3
"""
Dataset Export Script - v3.0
Exports annotated frames + Ring snapshots + negatives to YOLO format
with CLAHE preprocessing and stratified splits.

Improvements over v2.0:
1. Fixes bbox coordinate handling — reads actual image dimensions instead of hardcoded 1920x1080
2. Additional snapshot search paths (dell-deployment/data/snapshots)
3. More confirmed non-deer negatives from Ring snapshots (1000+)
...
Output:
    data/training_datasets/v3.0_YYYYMMDD/
...
SNAPSHOTS_DIR_ALT = Path("/home/rndpig/deer-deterrent/dell-deployment/data/snapshots")
...
def bbox_detection_to_yolo(bbox: Dict, img_width: int = 640, img_height: int = 360) -> str:
    """
    ...
    Note: img_width/img_height must match the actual image dimensions.
    Ring snapshots are 640x360; manual uploads may differ.
    """
...
def export_dataset():
    """Main export function — v3.0"""
    
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    OUTPUT_DIR = OUTPUT_BASE / f"v3.0_{timestamp}"
    
    print("=" * 80)
    print(f"EXPORTING DATASET v3.0 — {timestamp}")
    print("=" * 80)
    ...
    for snap in deer_snapshots:
        ...
        candidates = [
            ...
            SNAPSHOTS_DIR_ALT / Path(snap_path).name,
            Path(snap_path),
        ...
        # Read actual image dimensions for correct bbox normalization
        img_w, img_h = 640, 360  # Default for Ring snapshots
        if HAS_CV2:
            img = cv2.imread(str(source_path))
            if img is not None:
        ...
        if snap['detection_bboxes']:
            try:
                ...
                for bbox_entry in bboxes:
                    ...
                    if bbox_dict:
                        line = bbox_detection_to_yolo(bbox_dict, img_width=img_w, img_height=img_h)
                        if validate_yolo_line(line):
                            yolo_lines.append(line)
    ...
    with open(yaml_path, 'w') as f:
        f.write(f"# Deer Deterrent Dataset v3.0\n")
    ...
    print(f"\n  Next steps:")
    print(f"    1. Zip: cd {OUTPUT_DIR} && tar czf ../dataset_v3.0.tar.gz .")
